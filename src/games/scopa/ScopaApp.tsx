 ...
 */
function loadSpectatorAIs(): { player1: AIType; player2: AIType } {
  try {
...
function ScopaApp() {
  ...
  const statsModalOpponents: StatsModalOpponent[] = useMemo(() => {
    return getAllDisplayOpponents().map((o) => {
      const s = getOpponentStats(o.type);
      return {
        key: o.type,
        label: (
          <AIPlayerLabel
  ...
  const getStatsModalGames = useCallback(
    (key: string): StatsModalGame[] => {
      const games = getGamesAgainst(key as AIType);
      return games.map((g) => ({
        id: g.id,
  ...
  const [spectatorAIs, setSpectatorAIs] = useState<{ player1: AIType; player2: AIType }>(loadSpectatorAIs);
  ...
  const isSyncAI = useCallback((aiType: AIType): aiType is CPUType => {
  ...
  const getAIPlayer = useCallback((aiType: AIType) => {
  ...
  // Track last move made by each player
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(async () => {
      ...
      // NOTE: expertAI.selectMove(context) is only the heuristic
      // FALLBACK (used when no GameState is available) — the real
      // Esperto (ISMCTS + perfect-info endgame solver) lives in
      // selectExpertMoveWithState(state). Route 'expert' through that
      // directly so endgame play is actually optimal.
      if (aiType === 'expert') {
  ...
  }, [state.round.currentPlayer, state.status, state.players.cpu.hand, state.round.table, playCard, animatingCard, settings.cpuAI, isSpectatorMode, isSpectatorPaused, spectatorAIs.player2, scopaCelebration.show, setteBelloCelebration.show, isDealing, getAnimationDelay, getAIPlayer, playSound]);
  ...
  const handleStartGame = useCallback((targetScore: number, gameMode: 'pvsCPU' | 'cpuVsCPU') => {
    ...
    if (gameMode === 'cpuVsCPU' && canUseWorker && settings.animationSpeed === 'instant') {
      ...
      // Use main thread for animations or player vs CPU
      setUseWorkerMode(false);
      startGame(targetScore, gameMode);
    }
  }, [startGame, canUseWorker, startSimulation, spectatorAIs, settings.animationSpeed]);
  ...
  }, [activeState.status, workerIsRunning, useWorkerMode, stopSimulation, resetGame]);
  ...
  const confirmAndStartNewGame = useCallback(() => {
    ...
    resetGame();
  }, [useWorkerMode, stopSimulation, resetGame]);
  ...
  useEffect(() => {
    ...
    if (activeState.status === 'gameEnd' && !gameRecorded.current) {
      ...
      trackGameCompleted({ mode: 'solo', opponent: 'cpu' });
  ...
  }, [activeState.status, activeState.gameMode, activeState.scores, activeState.roundNumber, activeState.targetScore, settings.cpuAI, recordGame, playSound]);
  ...
  }, [isSpectatorMode, isSpectatorPaused, state.round.currentPlayer, state.status, state.players.human.hand, state.round.table, spectatorAIs.player1, playCard, animatingCard, scopaCelebration.show, setteBelloCelebration.show, isDealing, getAnimationDelay, getAIPlayer, playSound]);
